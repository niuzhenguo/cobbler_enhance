...
def console_page(request):
   """
   This page is used to redirect console to the user.
   """
   if not test_user_authenticated(request): return login(request)
   # FIXME: test and make sure we use this rather than throwing lots of tracebacks for
   # field errors
   t = get_template('console_page.tmpl')
   html = t.render(RequestContext(request,{
       'version' : remote.extended_version(request.session['token'])['version'],
       'username': username
   }))
...
def rebuild_page(request,obj_name=None):
   """
   This page is used to rebuild system.
   """
   if not test_user_authenticated(request): return login(request)
   # FIXME: test and make sure we use this rather than throwing lots of tracebacks for
   # field errors
   t = get_template('rebuild_page.tmpl')
   html = t.render(RequestContext(request,{
       'version' : remote.extended_version(request.session['token'])['version'],
       'obj_name': obj_name,
       'username': username
...
def genlist(request, what, page=None):
    ...
    if what == "system":
       ...
       columns = [ "name", "profile", "status", "comment", "netboot_enabled" ]
       batchactions += [
           ["Power on","power","on"],
           ["Power off","power","off"],
           ["Reboot","power","reboot"],
...
@require_POST
@csrf_protect
def generic_console(request, what, obj_name=None):
   """
   connect to remote console of an object.
   """
   if not test_user_authenticated(request): return login(request, next="/cobbler_web/%s/console/%s" % (what,obj_name))
   # FIXME: consolidate code with above functions.
   if obj_name == None:
      return error_page(request,"You must specify a %s to connect" % what)
   if not remote.has_item(what,obj_name):
      return error_page(request,"Unknown %s specified" % what)
   elif not remote.check_access_no_fail(request.session['token'], "modify_%s" % what, obj_name):
      return error_page(request,"You do not have permission to connect this %s" % what)
   else:
      ...
      if (power_address and power_user and power_pass):
         #return error_page(request, "power_address: %s power_user: %s power_pass: %s" % (power_address, power_user, power_pass))
         cmd = "/usr/local/bin/jviewer %s %s %s" % (power_address, power_user, power_pass)
         os.system(cmd)
      else:
         return error_page(request, "can not getpower infomations.")
...
@require_POST
@csrf_protect
def generic_bios(request, what, obj_name=None):
   """
   Force an object boot into bios.
   """
   if not test_user_authenticated(request): return login(request, next="/cobbler_web/%s/bios/%s" % (what,obj_name))
   # FIXME: consolidate code with above functions.
   if obj_name == None:
      return error_page(request,"You must specify a %s to set" % what)
   if not remote.has_item(what,obj_name):
      return error_page(request,"Unknown %s specified" % what)
   elif not remote.check_access_no_fail(request.session['token'], "modify_%s" % what, obj_name):
      return error_page(request,"You do not have permission to set this %s" % what)
   else:
      ...
      if (power_address and power_user and power_pass):
         #return error_page(request, "power_address: %s power_user: %s power_pass: %s" % (power_address, power_user, power_pass))
         cmd = "/usr/bin/ipmitool -I lan -H %s -U %s -P %s chassis bootdev bios" % (power_address, power_user, power_pass)
         if (os.system(cmd)):
             return error_page(request, "set bootdev bios failed.")
      else:
         return error_page(request, "can not getpower infomations.")
...
@require_POST
@csrf_protect
def generic_pxe(request, what, obj_name=None):
   """
   Force an object boot pxe.
   """
   if not test_user_authenticated(request): return login(request, next="/cobbler_web/%s/pxe/%s" % (what,obj_name))
   # FIXME: consolidate code with above functions.
   if obj_name == None:
      return error_page(request,"You must specify a %s to set" % what)
   if not remote.has_item(what,obj_name):
...
@require_POST
@csrf_protect
def generic_enpxe(request, what, obj_name=None):
   """
   enable pxe installation of an object.
   """
   if not test_user_authenticated(request): return login(request, next="/cobbler_web/%s/enpxe/%s" % (what,obj_name))
   # FIXME: consolidate code with above functions.
   if obj_name == None:
      return error_page(request,"You must specify a %s to connect" % what)
   if not remote.has_item(what,obj_name):
      ...
      remote.modify_system(obj_id, "netboot_enabled", True, request.session['token'])
      remote.save_system(obj_id, request.session['token'], "edit")
      '''

      return rebuild_page(request,obj_name)

      #return HttpResponseRedirect("/cobbler_web/%s/rebuild/%s" % (what,obj_name))

# ======================================================================

@require_POST
@csrf_protect
def generic_rebuild(request, what, obj_name=None):
   """
   Force to rebuild a system.
   """
   if not test_user_authenticated(request): return login(request, next="/cobbler_web/%s/rebuild/%s" % (what,obj_name))
   # FIXME: consolidate code with above functions.
   if obj_name == None:
      return error_page(request,"You must specify a %s to set" % what)
   if not remote.has_item(what,obj_name):
      ...
      # Step1: Netboot enabled
      remote.modify_system(obj_id, "netboot_enabled", True, request.session['token'])
      remote.save_system(obj_id, request.session['token'], "edit")

      # Step2: Run 'cobbler sync'
      remote.background_sync({"verbose":"True"},request.session['token'])

      if (power_address and power_user and power_pass):
         # Step3: Enable boot pxe.
         cmd = "/usr/bin/ipmitool -I lan -H %s -U %s -P %s chassis bootdev pxe" % (power_address, power_user, power_pass)
         if (os.system(cmd)): 
             return error_page(request, "set bootdev pxe failed.")

         # Step4: power on or reset node
         ...
         return error_page(request, "can not get power infomations.")
...
@require_POST
@csrf_protect
def generic_power(request, what, obj_name=None):
   """
   Force to power a system.
   """
   if not test_user_authenticated(request): return login(request, next="/cobbler_web/%s/rebuild/%s" % (what,obj_name))
   # FIXME: consolidate code with above functions.
   if obj_name == None:
